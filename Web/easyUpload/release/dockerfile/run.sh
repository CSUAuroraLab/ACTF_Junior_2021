...
echo "**************** las adding jobs start  *****************"
echo " 1. make directory /app/upload"
mkdir /app/upload

echo " 2. change owners of /app and /app/upload"
chown root:www-data /app
chown www-data:www-data /app/upload
chown root:root /app/index.php
chown root:root /app/index.html

echo " 3. run rmUploadFiles.sh in the background"
nohup /rmUploadFiles.sh &
rm /nohup.out

echo " 4. delete the shell script in the / directory"
rm /rmUploadFiles.sh
rm /run.sh

echo "done!"
echo "**************** las adding jobs stop   *****************"
